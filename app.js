...
(function() {
  var BOARD_SIZE, Board, SIDE_LENGTH, alphabeta, b, checkWin, max, maxIndex, min, minIndex, minimax;
  ...
  alphabeta = function(board, curPlayer, depth, alpha, beta, cache) {
    var abResult, bestScore, boardKey, elem, index, k, len, newBoard, scores, winner;
    winner = checkWin(board);
    if (winner === 1) {
      return BOARD_SIZE - depth;
    }
    ...
    bestScore = curPlayer === 1 ? -Infinity : Infinity;
    for (index = k = 0, len = board.length; k < len; index = ++k) {
      elem = board[index];
      if (elem) {
        continue;
    ...
    Board.prototype.suggestMove = (function() {
      ...
      return function() {
        ...
        for (index = k = 0, len = ref.length; k < len; index = ++k) {
          ...
          scores[index] = alphabeta(newBoard, this.curPlayer % 2 + 1, this.pastMoves.length, -Infinity, Infinity, cache);
        }
